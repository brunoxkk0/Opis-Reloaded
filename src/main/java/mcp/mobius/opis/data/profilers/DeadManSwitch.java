...
public class DeadManSwitch implements Runnable {
    ...
    private final AtomicLong timerPrev = new AtomicLong();
                ...
                if (this.lastTested != this.timer.get()) {
                    ...
                    if (this.getTimerDelta() / 1000. / 1000. > 300.) {
                        ...
                        if (this.nderps > 10) {
                            ...
                            return;
                             */
                            Thread.getAllStackTraces().keySet().stream().filter((thread) -> (thread != Thread.currentThread())).forEachOrdered((thread) -> {
                                try {
                                    thread.interrupt();
                                } catch (Exception e) {
                                    System.out.printf("%s\n", e);

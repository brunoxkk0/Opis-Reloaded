...
public class DataChunk implements ISerializable, Comparable {
    ...
    public DataChunk fill(CoordinatesChunk chunk) {
        ...
        totalUpdate = tileEnts.stream().map((tileent) -> tileent.update.timing).reduce(totalUpdate, (accumulator, _item) -> accumulator + _item);

        totalUpdate = entities.stream().map((ent) -> ent.update.timing).reduce(totalUpdate, (accumulator, _item) -> accumulator + _item);

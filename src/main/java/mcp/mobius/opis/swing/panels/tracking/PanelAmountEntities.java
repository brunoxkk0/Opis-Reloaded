...
public class PanelAmountEntities extends JPanelMsgHandler implements ITabPanel {
    ...
    private final JCheckBox chckbxFilter;
    private final JButtonAccess btnKillAll;
    private final JButtonAccess btnRefresh;
    ...
    public boolean handleMessage(Message msg, PacketBase rawdata) {
        switch (msg) {
            case LIST_AMOUNT_ENTITIES: {
                ...
                totalEntities = rawdata.array.stream().map((o) -> (AmountHolder) o).map((entity) -> {
                    model.addRow(new Object[]{entity.key, entity.value});
                    return entity;
                }).map((entity) -> entity.value).reduce(totalEntities, Integer::sum);

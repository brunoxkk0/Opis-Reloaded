...
public class PanelSummary extends JPanelMsgHandler implements ITabPanel {
    ...
    private final JLabel lblTimingWorldTick;
    private final JLabel lblTimingTileEnts;
    private final JLabel lblTimingEntities;
    private final JLabel lblTimingTotal;
    private final JLabel lblTickTime;
    private final JLabel lblAmountTileEnts;
    private final JLabel lblAmountEntities;
    private final JLabel lblAmountUpload;
    private final JLabel lblAmountDownload;
    private final JLabel lblAmountForced;
    private final JLabel lblAmountLoaded;
    private final JButtonAccess btnRun;
    private final JProgressBar progressBarRun;
    ...
    private final JLabel lblTimingPing;
    ...
    public boolean handleMessage(Message msg, PacketBase rawdata) {
        switch (msg) {
            ...
            case VALUE_TIMING_WORLDTICK: {
                ...
                tooltip = ((DataBlockTick) rawdata.value).perdim.keySet().stream().map((indim) -> String.format("[ %4d ] %s<br>", indim, ((DataBlockTick) rawdata.value).perdim.get(indim).asMillisecond().toString())).reduce(tooltip, String::concat); //System.out.printf("WorldTick [ %4d ] %s\n", indim, ((DataBlockTick)rawdata.value).perdim.get(indim).asMillisecond().toString());
